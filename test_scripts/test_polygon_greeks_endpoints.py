#!/usr/bin/env python3
"""
Simple Polygon Greeks Endpoints Tester

This script tests Polygon API endpoints to see what Greeks data is available
without requiring any database connections.
"""
...
@dataclass
class EndpointTest:
    """Container for endpoint test results"""
    endpoint: str
    method: str
    params: Dict[str, Any]
    status_code: int
...
class PolygonGreeksTester:
    """Simple tester for Polygon Greeks endpoints"""
    ...
    def test_all_endpoints(self, symbols: List[str] = None) -> List[EndpointTest]:
        ...
        for symbol in symbols:
            ...
            # Test 4: Previous close endpoint (for underlying price)
            self._test_previous_close_endpoint(symbol)
            
            # Test 5: Aggregates endpoint (for historical price data)
    ...
    def _test_previous_close_endpoint(self, symbol: str):
        ...
        logger.info("  4. Testing previous close endpoint...")
    ...
    def _test_aggregates_endpoint(self, symbol: str):
        ...
        logger.info("  5. Testing aggregates endpoint...")
    ...
    def generate_report(self) -> str:
        ...
        successful_tests = len([t for t in self.test_results if t.status_code == 200])
        data_found_tests = len([t for t in self.test_results if t.data_found])
        greeks_found_tests = len([t for t in self.test_results if t.greeks_found])
...
def main():
    ...
    try:
        ...
        test_results = tester.test_all_endpoints(test_symbols)
        ...
        successful_tests = len([t for t in test_results if t.status_code == 200])
        greeks_found = len([t for t in test_results if t.greeks_found])
        
        logger.info(f"📊 Results:")
        logger.info(f"  - Endpoint tests: {len(test_results)}")
