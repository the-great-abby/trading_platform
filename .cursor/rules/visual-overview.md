# Visual Overview: Cursor Rules System

## System Architecture

The cursor rules system uses a hierarchical approach with automatic file pattern matching and dynamic style switching.
...
When you open a file in Cursor, the system automatically determines which rules to apply:
...
You can switch the AI's response style instantly without creating new files:
...
The system covers all aspects of your development workflow:
...
See how the rules integrate with your daily development workflow:
...
The optimized structure provides significant improvements:
...
### Rule Application
- **Core rules** apply to all files automatically
- **Specialized rules** apply based on file type
- **Style rules** can be switched dynamically
- **No restart required** - changes apply immediately

### File Type Coverage
- **Python files** → `core.mdc` + `python.mdc`
- **Docker/K8s files** → `core.mdc` + `infrastructure.mdc`
- **Database files** → `core.mdc` + `data.mdc`
- **Trading files** → `core.mdc` + `trading.mdc`
- **All files** → `core.mdc` + current style

This visual system makes it easy to understand how the cursor rules work together to provide context-aware, intelligent assistance for your trading system development. 