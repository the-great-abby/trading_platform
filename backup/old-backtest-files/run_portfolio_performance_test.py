...
and analyzes portfolio performance across different strategies.
"""
...
class PortfolioPerformanceAnalyzer:
    """Analyzes portfolio performance across multiple strategies."""
    ...
    async def run_portfolio_backtest(self) -> Dict[str, Any]:
        """Run comprehensive portfolio backtest across all strategies."""
        
        logger.info("🚀 Starting Portfolio Performance Test")
        logger.info(f"📊 Testing {len(self.symbols)} symbols")
        logger.info(f"📅 Date range: 2023-07-10 to 2025-07-09")
        
        # Define strategies to test (excluding Greeks strategy)
        strategies = {
    ...
    def _generate_portfolio_report(self, portfolio_summary: Dict[str, Any], strategy_results: Dict[str, Any]):
        ...
        logger.info("\n" + "="*80)
        logger.info("✅ Portfolio Performance Test Completed!")
        logger.info("="*80)

async def main():
    """Main function to run portfolio performance test."""
    
    analyzer = PortfolioPerformanceAnalyzer()
    
    try:
        results = await analyzer.run_portfolio_backtest()
        logger.info("🎉 Portfolio performance test completed successfully!")
        
    except Exception as e:
        logger.error(f"❌ Portfolio performance test failed: {e}")
        raise
