...
from ..services.market_data.cached_market_data_manager import get_cached_market_data_manager
...
class BacktestEngine:
    ...
    async def _backtest_symbol(self, strategy: BaseStrategy, symbol: str, data: pd.DataFrame) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for i in range(len(data)):
                ...
                if signal:
                    signals_generated += 1
                    logger.info(f"🔍 DEBUG: {symbol} - Signal {signals_generated}: {signal.action} at {data.index[i]}")
                    
                    # Execute trade
                    if signal.action == 'BUY' and position == 0:
                        # Buy signal
                        shares = int(current_capital / data.iloc[i]['Close'])
                        if shares > 0:
                            position = shares
                            trade_cost = shares * data.iloc[i]['Close']
                            ...
                            logger.info(f"🔍 DEBUG: {symbol} - BUY trade: {shares} shares at ${data.iloc[i]['Close']:.2f}")
                    
                    elif signal.action == 'SELL' and position > 0:
                        # Sell signal
                        trade_value = position * data.iloc[i]['Close']
                        current_capital += trade_value
                        
                        # Calculate P&L
                        buy_trade = trades[-1] if trades else None
                        if buy_trade:
                            pnl = trade_value - buy_trade['cost']
                            trades.append({
                                'date': data.index[i],
                                'action': 'SELL',
                                'price': data.iloc[i]['Close'],
                            ...
                            logger.info(f"🔍 DEBUG: {symbol} - SELL trade: {position} shares at ${data.iloc[i]['Close']:.2f}, P&L: ${pnl:.2f}")
            ...
            if position > 0:
                ...
                logger.info(f"🔍 DEBUG: {symbol} - Closing position: {position} shares at ${data.iloc[-1]['Close']:.2f}")
    ...
    def _get_strategy_class(self, strategy_name: str):
        ...
        strategy_map = {
            ...
            'bollinger_bands': 'src.strategies.bollinger_bands_strategy.BollingerBandsStrategy',
            'news_enhanced': 'src.strategies.news_enhanced_strategy.NewsEnhancedStrategy',
            'momentum': 'src.strategies.momentum_strategy.MomentumStrategy',
            'mean_reversion': 'src.strategies.mean_reversion_strategy.MeanReversionStrategy',
            'volatility_breakout': 'src.strategies.volatility_breakout_strategy.VolatilityBreakoutStrategy',
            'portfolio': 'src.strategies.portfolio_strategy.PortfolioStrategy',
            # Add uppercase mappings for compatibility
            'SMACrossoverStrategy': 'src.strategies.sma_crossover.SMACrossoverStrategy',
            'RSIStrategy': 'src.strategies.rsi_strategy.RSIStrategy',
            'MACDStrategy': 'src.strategies.macd_strategy.MACDStrategy',
            'BollingerBandsStrategy': 'src.strategies.bollinger_bands_strategy.BollingerBandsStrategy',
            'NewsEnhancedStrategy': 'src.strategies.news_enhanced_strategy.NewsEnhancedStrategy',
            'MomentumStrategy': 'src.strategies.momentum_strategy.MomentumStrategy',
            'MeanReversionStrategy': 'src.strategies.mean_reversion_strategy.MeanReversionStrategy',
            'VolatilityBreakoutStrategy': 'src.strategies.volatility_breakout_strategy.VolatilityBreakoutStrategy',
            'PortfolioStrategy': 'src.strategies.portfolio_strategy.PortfolioStrategy',
            # Portfolio configurations
            'Conservative_Portfolio': 'src.strategies.portfolio_strategy.PortfolioStrategy',
            'Balanced_Portfolio': 'src.strategies.portfolio_strategy.PortfolioStrategy',
            'Aggressive_Portfolio': 'src.strategies.portfolio_strategy.PortfolioStrategy',
            'Risk_Averse_Portfolio': 'src.strategies.portfolio_strategy.PortfolioStrategy'
        }
