...
async def main():
    ...
    try:
        ...
        logger.info("📊 Backtest Configuration (AGGRESSIVE):")
        logger.info(f"   • Symbols: {symbols}")
        logger.info(f"   • Period: {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}")
        logger.info(f"   • Initial Capital: ${engine.initial_capital:,.2f}")
        logger.info(f"   • Cash Reserve: {engine.cash_reserve_pct:.1%} (AGGRESSIVE)")
        logger.info(f"   • Max Position Size: {engine.max_position_size_pct:.1%} (AGGRESSIVE)")
        ...
        # Display results
        logger.info("\n📈 MULTI-STRATEGY ENSEMBLE RESULTS (AGGRESSIVE ALLOCATION)")
        logger.info("=" * 80)
        ...
        logger.info(f"   • Current Aggressive Result: {ensemble_result.total_return_pct:.2f}%")
        if ensemble_result.total_return_pct >= 1139.36:
            logger.info(f"   • 🎉 EXCEEDED CONSERVATIVE PERFORMANCE by {ensemble_result.total_return_pct - 1139.36:.2f} percentage points!")
        else:
            logger.info(f"   • ⚠️ Below conservative performance by {1139.36 - ensemble_result.total_return_pct:.2f} percentage points.")
        logger.info("")
