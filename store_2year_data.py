...
async def store_2year_data():
    """Store 2 years of historical data for all symbols"""
    config = get_config()
    ...
    print(f"📊 Storing 2 years of historical data for {len(symbols)} symbols")
    ...
    for i, symbol in enumerate(symbols, 1):
        ...
        try:
            ...
            if data is not None and not data.empty:
                ...
                stored_count = market_data_service.store_historical_data(symbol, data, "market_data_manager", "1d")
                
                symbol_time = time.time() - symbol_start_time
                successful_symbols += 1
                total_stored += len(data)
                
                print(f"   ✅ Success: {len(data)} records fetched, {len(data)} stored in {symbol_time:.2f}s")
                if len(data) > 0:
