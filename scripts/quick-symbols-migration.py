#!/usr/bin/env python3
"""
Quick Migration Script: Restore Missing 33 Symbols
Migrates popular_symbols table from old TimescaleDB to external config database
"""
...
def migrate_popular_symbols():
    """Migrate popular_symbols table from old to new database"""
    ...
    # Target: External Config Database
    target_config = {
        "host": "localhost",
        "port": 11153,
        "database": "trading",
    ...
    try:
        ...
        with target_conn.cursor() as cur:
            ...
            target_conn.commit()
            logger.info(f"✅ Successfully migrated {len(symbols)} symbols to target database")
        
        # Verify migration
        with target_conn.cursor() as cur:
            cur.execute("SELECT COUNT(*) FROM popular_symbols")
            target_count = cur.fetchone()[0]
            logger.info(f"🔍 Target database now has {target_count} symbols")
            
            if source_count == target_count:
                logger.info("🎉 Migration successful! All symbols restored.")
            else:
...
if __name__ == "__main__":
    logger.info("🚀 Starting quick symbols migration...")
    migrate_popular_symbols()
