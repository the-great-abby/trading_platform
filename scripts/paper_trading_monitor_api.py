#!/usr/bin/env python3
"""
Paper Trading Monitor - Updated to fetch data from database via API
...
class PaperTradingMonitor:
    """Real-time paper trading monitor that fetches data from paper trading service API"""
    ...
    def __init__(self, refresh_interval: int = 5):
        ...
        self.paper_service_url = "http://localhost:11190"
        ...
        logger.info(f"🔗 Connected to paper trading service: {self.paper_service_url}")
    
    async def fetch_orders_from_api(self) -> List[Dict]:
        """Fetch orders from paper trading service API"""
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"{self.paper_service_url}/api/v1/trading/orders",
    ...
    async def fetch_positions_from_api(self) -> List[Dict]:
        """Fetch positions from paper trading service API"""
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"{self.paper_service_url}/api/v1/trading/positions",
                    params={"account_id": self.account_id},
                    timeout=aiohttp.ClientTimeout(total=10)
                ) as response:
                    if response.status == 200:
    ...
    async def fetch_account_info_from_api(self) -> Dict:
        """Fetch account information from paper trading service API"""
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"{self.paper_service_url}/api/v1/accounts/{self.account_id}",
                    timeout=aiohttp.ClientTimeout(total=10)
                ) as response:
                    if response.status == 200:
                        data = await response.json()
    ...
    async def update_from_api(self):
        """Update monitor data from paper trading service API"""
        try:
    ...
    def get_status(self) -> Dict:
        ...
        return {
            ...
            'paper_service_url': self.paper_service_url
        }
    ...
    def print_status(self):
        ...
        print(f"🔗 Data Source: Paper Trading Service API ({status['paper_service_url']})")
