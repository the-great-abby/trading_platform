...
class StrategyOptimizer:
    ...
    def generate_optimization_report(self) -> Dict:
        ...
        total_strategies = len(self.strategy_metrics)
        avg_sharpe = np.mean([m.sharpe_ratio for m in self.strategy_metrics.values()])
        avg_win_rate = np.mean([m.win_rate for m in self.strategy_metrics.values()])
        avg_drawdown = np.mean([m.max_drawdown for m in self.strategy_metrics.values()])
