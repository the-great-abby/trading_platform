...
if [ $? -eq 0 ] && [ -n "$CURRENT_SYMBOLS" ]; then
    ...
    SYMBOLS_TO_DELETE=$(echo "$CURRENT_SYMBOLS" | grep -v "TEST")
    if [ -n "$SYMBOLS_TO_DELETE" ]; then
        echo "🗑️  Deleting existing symbols..."
        for symbol in $SYMBOLS_TO_DELETE; do
    ...
    echo "📊 No existing symbols found or error getting symbols"
fi

# Central symbol list with categories and descriptions
echo ""
...
if [ $SUCCESSFUL_ADDS -eq 41 ]; then
    ...
    echo ""
    echo "⚠️  Some symbols failed to add. Check the errors above."
fi
