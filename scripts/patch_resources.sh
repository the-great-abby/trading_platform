...
print_status "Patching backtest-api (7m CPU, 44Mi Memory → 20m CPU, 100Mi Memory)"
...
print_status "Patching backtest-request-service (5m CPU, 50Mi Memory → 15m CPU, 100Mi Memory)"
...
print_status "Patching grafana (17m CPU, 116Mi Memory → 40m CPU, 250Mi Memory)"
kubectl patch deployment grafana -n trading-system -p '{"spec":{"template":{"spec":{"containers":[{"name":"grafana","resources":{"requests":{"cpu":"40m","memory":"250Mi"},"limits":{"cpu":"80m","memory":"500Mi"}}}]}}}}'

print_status "Patching market-data-service (5m CPU, 116Mi Memory → 15m CPU, 250Mi Memory)"
...
kubectl patch deployment postgres-dev -n trading-system -p '{"spec":{"template":{"spec":{"containers":[{"name":"postgres-dev","resources":{"requests":{"cpu":"5m","memory":"50Mi"},"limits":{"cpu":"10m","memory":"100Mi"}}}]}}}}'

print_status "Patching rabbitmq (9m CPU, 165Mi Memory → 20m CPU, 350Mi Memory)"
kubectl patch deployment rabbitmq -n trading-system -p '{"spec":{"template":{"spec":{"containers":[{"name":"rabbitmq","resources":{"requests":{"cpu":"20m","memory":"350Mi"},"limits":{"cpu":"40m","memory":"700Mi"}}}]}}}}'
...
print_status "Patching unified-analytics-dashboard (5m CPU, 52Mi Memory → 15m CPU, 100Mi Memory)"
...
print_status "Patching unified-trading-dashboard (7m CPU, 52Mi Memory → 15m CPU, 100Mi Memory)"
kubectl patch deployment unified-trading-dashboard -n trading-system -p '{"spec":{"template":{"spec":{"containers":[{"name":"unified-trading-dashboard","resources":{"requests":{"cpu":"15m","memory":"100Mi"},"limits":{"cpu":"30m","memory":"200Mi"}}}]}}}}'
...
print_status "Patching metrics-test-service (4m CPU, 134Mi Memory → 10m CPU, 300Mi Memory)"
