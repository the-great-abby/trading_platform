...
- PARALLEL EXECUTION: Added fast backtest features for speed
"""
...
class TwoYearLLMExtremeAggressiveBacktest:
    """2-Year LLM-Enhanced Backtest with Extreme Aggressive Risk Settings for Small Accounts"""
    
    def __init__(self, initial_capital: float = 1000.0, use_parallel: bool = True, max_workers: int = 8, symbol_limit: int = 100):
        ...
        # ULTRA AGGRESSIVE Position sizing for small account
        self.max_position_size = 0.30  # Max 30% of portfolio per position (was 15%)
        self.min_trade_value = 50.0    # Minimum $50 per trade (was $100) - LOWER MINIMUM
        self.max_trade_value = 300.0   # Maximum $300 per trade (was $150) - 30% of $1K
        
        # Realistic trading features - ULTRA AGGRESSIVE
        self.commission_per_trade = 1.0  # $1 commission per trade
        self.slippage_percentage = 0.02  # 0.02% slippage on trades (was 0.05%)
        self.partial_fill_probability = 0.05  # 5% chance of partial fill (was 15%)
        self.market_hours_only = True  # Only trade during market hours
        self.max_daily_trades = 10  # Max 10 trades per day (was 3) - TRIPLE
        self.min_holding_period = 1  # Minimum 1 day holding period
        self.stop_loss_percentage = 0.15  # 15% stop loss (was 8%)
        self.take_profit_percentage = 0.25  # 25% take profit (was 15%)
        
        # ULTRA AGGRESSIVE Risk management
        self.max_daily_loss = 150.0  # Max $150 loss per day (was $50)
        self.max_portfolio_risk = 0.20  # Max 20% portfolio risk per trade (was 10%)
        self.correlation_limit = 0.9  # Max correlation between positions (was 0.7)
        ...
        # Get symbols from centralized configuration with optional limiting
        self.all_symbols = get_symbols()[:self.symbol_limit] if self.symbol_limit else get_symbols()
        self.options_symbols = get_options_symbols()[:self.symbol_limit] if self.symbol_limit else get_options_symbols()
        
        logger.info(f"📊 Using centralized symbol configuration:")
        logger.info(f"   Standard symbols: {len(self.all_symbols)} (limited to {self.symbol_limit} for speed)" if self.symbol_limit else f"   Standard symbols: {len(self.all_symbols)}")
        logger.info(f"   Options symbols: {len(self.options_symbols)} (limited to {self.symbol_limit} for speed)" if self.symbol_limit else f"   Options symbols: {len(self.options_symbols)}")
        logger.info(f"   Parallel execution: {'Enabled' if self.use_parallel else 'Disabled'}")
        ...
        # LLM configuration - ULTRA AGGRESSIVE
        self.llm_timeout = 10.0  # seconds
        self.llm_retry_attempts = 2
        self.fallback_confidence = 0.01  # EXTREMELY LOW confidence threshold (was 0.1)
    ...
    def _calculate_portfolio_performance(self):
        ...
        self.portfolio_stats['final_portfolio_value'] = float(final_value)
    ...
    async def _run_strategies_with_llm(self, engine: BacktestEngine, symbols: List[str], 
        ...
        if self.use_parallel:
            # Use optimized backtest engine for parallel execution
            return await self._run_strategies_parallel(symbols, strategies, strategy_type)
        else:
            # Use standard backtest engine for sequential execution
            return await self._run_strategies_sequential(engine, symbols, strategies, strategy_type)
    
    async def _run_strategies_parallel(self, symbols: List[str], strategies: List[str], strategy_type: str) -> Dict[str, Any]:
        """Run strategies with parallel execution using OptimizedBacktestEngine"""
        
        logger.info(f"⚡ Running {strategy_type} strategies with PARALLEL execution...")
        ...
        backtest_config = BacktestConfig(
            ...
            batch_size=5,
            timeout_hours=24,  # Longer timeout for 2-year backtest
            use_llm=True  # Enable LLM evaluation
        )
        
        # Initialize optimized engine
        optimized_engine = OptimizedBacktestEngine(backtest_config)
        
        # Run optimized backtest
        results = await optimized_engine.run_optimized_backtest()
        
        logger.info(f"✅ Parallel {strategy_type} strategies completed")
        return results
    
    async def _run_strategies_sequential(self, engine: BacktestEngine, symbols: List[str], 
                                       strategies: List[str], strategy_type: str) -> Dict[str, Any]:
        """Run strategies with sequential execution using standard BacktestEngine"""
        
        logger.info(f"🏃 Running {strategy_type} strategies with SEQUENTIAL execution...")
        ...
        logger.info(f"✅ Sequential {strategy_type} strategies completed")
        return results
...
class DollarTrackingTradeEvaluator(TradeEvaluator):
    ...
    async def evaluate_trade_signal(self, signal, market_data: pd.DataFrame, 
        ...
        if not self.llm_available or self.ollama_service is None:
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='2-Year LLM Backtest with Parallel Execution')
    parser.add_argument('--no-parallel', action='store_true', help='Disable parallel execution')
    parser.add_argument('--max-workers', type=int, help='Maximum number of workers')
    parser.add_argument('--symbol-limit', type=int, help='Limit number of symbols for speed')
    ...
    # Initialize and run backtest with parallel options
    backtest = TwoYearLLMExtremeAggressiveBacktest(
