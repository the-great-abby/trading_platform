...
class ElliottWaveImpulseStrategy(BaseStrategy):
    ...
    def __init__(self, 
                 ...
                 lookback_period: int = 10,  # Reduced further
                 momentum_threshold: float = 0.001,  # Much more aggressive
                 **kwargs):
    ...
    def analyze_impulse_pattern(self, swing_points: Dict[str, List], data: pd.DataFrame) -> Dict[str, Any]:
        """Analyze for impulse patterns with improved logic"""
        ...
        # Look for trending patterns using multiple methods
        ...
        # Method 3: Recent swing analysis
        recent_highs = highs[-3:] if len(highs) >= 3 else highs
        recent_lows = lows[-3:] if len(lows) >= 3 else lows
        
        swing_trend = 0.0
        if len(recent_highs) >= 2 and len(recent_lows) >= 2:
            high_trend = (recent_highs[-1]['price'] - recent_highs[0]['price']) / recent_highs[0]['price']
            low_trend = (recent_lows[-1]['price'] - recent_lows[0]['price']) / recent_lows[0]['price']
            swing_trend = (high_trend + low_trend) / 2
        
        # Combine all signals
        total_trend = (momentum + ma_trend + swing_trend) / 3
        
        # Determine if it's an impulse pattern - OPTIMIZED TUNING
        if abs(total_trend) > self.momentum_threshold:
            # Improved confidence calculation with better scaling
            confidence = min(abs(total_trend) * 10, 1.0)  # Increased scaling factor from 5 to 10
            
            # Additional validation: ensure we have meaningful trends
            if abs(momentum) > 0.005 and abs(ma_trend) > 0.002:  # Minimum thresholds
                return {
                    'pattern_found': True,
                    'pattern_type': 'impulse',
                    'direction': 'bullish' if total_trend > 0 else 'bearish',
        ...
        # Fallback: Generate signals based on simple momentum if no complex pattern found
        if abs(momentum) > self.momentum_threshold * 2:  # More aggressive fallback
            confidence = min(abs(momentum) * 8, 0.8)  # Lower confidence for fallback
            return {
                'pattern_found': True,
                'pattern_type': 'momentum_fallback',
