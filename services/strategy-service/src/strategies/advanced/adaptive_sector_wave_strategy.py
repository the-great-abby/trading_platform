...
class AdaptiveSectorWaveStrategy(BaseStrategy):
    ...
    def _select_options_strategy_with_greeks(self, symbol: str, data: pd.DataFrame, conditions: Dict, available_cash: float = 4000) -> str:
        ...
        # Calculate maximum affordable position value (20% of capital per trade - PREVIOUS WORKING CALCULATION)
        max_position_value = available_cash * 0.20  # $800 with $4k capital (PREVIOUS WORKING CALCULATION)
    ...
    def _get_multi_strategy_confirmation(self, symbol: str, data: pd.DataFrame, signal_type: str) -> Dict:
        ...
        # 1. Ichimoku confirmation (skip async for now to avoid blocking)
        if self.confirmation_strategies['ichimoku']:
            try:
                # Skip Ichimoku for now to avoid async issues in synchronous context
                confirmations['ichimoku'] = True  # Assume positive for now
            except:
    ...
    async def _generate_strategy_signal(self, 
        ...
        # Use proper cash reserve calculation (5% cash reserve = 95% available)
        estimated_available_cash = self._estimated_capital * self._capital_adjustment_factor * 0.95  # Use 95% of estimated capital (5% cash reserve)
    ...
    def _generate_strangle_signal(self, symbol: str, data: pd.DataFrame, price: float, conditions: Dict, min_confidence: float = 0.7) -> Optional[TradeSignal]:
        """Generate Strangle signal (trending + impulse wave OR corrective with volatility)"""
        
        # Strangle: Buy OTM call + OTM put (cheaper than straddle, needs bigger move)
        # Best for: Strong trend expected, directional breakout, OR corrective with volatility
        ...
        # Also enter on corrective patterns with moderate volatility (FIXED!)
        elif elliott['pattern'] == 'corrective' and vol['ratio'] < 1.5 and elliott['confidence'] > 0.3:
            timestamp = data.index[-1] if hasattr(data.index[-1], 'to_pydatetime') else datetime.now()
            # Use risk-adjusted position sizing
            contracts = self._calculate_position_size(symbol, conditions, 4000)  # Assuming $4000 portfolio
