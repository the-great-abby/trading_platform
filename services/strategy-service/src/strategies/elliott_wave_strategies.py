...
from src.strategies.elliott_wave_fallback import generate_fallback_signal
...
class ElliottWaveCorrectiveStrategy(BaseStrategy):
    ...
    def __init__(self, 
                 ...
                 elliott_service_url: str = "http://localhost:11082",
                 confidence_threshold: float = 0.1,  # Much more aggressive
                 **kwargs):
        super().__init__(name=name, **kwargs)
        self.elliott_service_url = elliott_service_url
        self.confidence_threshold = confidence_threshold
    ...
    def is_corrective_pattern(self, pattern_data: Dict[str, Any]) -> bool:
        """Check if the pattern is a corrective pattern - much more permissive"""
        if not pattern_data:
            return False
        
        # If no pattern found, still consider it corrective for mean reversion
        if not pattern_data.get('pattern_found', False):
            return True  # More aggressive - treat as corrective
        
        pattern_type = pattern_data.get('pattern_type', '').lower()
        return ('corrective' in pattern_type or 'sideways' in pattern_type or 
                'range' in pattern_type or 'impulse' in pattern_type)  # Accept impulse too
    
    def calculate_signal_confidence(self, pattern_data: Dict[str, Any]) -> float:
        """Calculate signal confidence based on pattern strength - much more aggressive"""
        if not pattern_data:
            return 0.5  # Default confidence even without pattern
        
        # Base confidence from pattern strength
        confidence = pattern_data.get('confidence', 0.3)  # Default to 0.3 instead of 0.0
        
        # Boost confidence for corrective patterns
        if self.is_corrective_pattern(pattern_data):
            confidence *= 1.2  # Increased boost
        
        return min(confidence, 1.0)
    
    def get_signal_direction(self, pattern_data: Dict[str, Any]) -> Optional[str]:
        """Determine signal direction based on corrective pattern - more aggressive"""
        if not pattern_data:
            return 'BUY'  # Default to BUY if no pattern data
        ...
        # If no clear direction, default to BUY
        return 'BUY'
    
    async def generate_signal(self, symbol: str, data: pd.DataFrame, historical_date: Optional[str] = None) -> Optional[TradeSignal]:
        """Generate Elliott Wave corrective trading signal"""
        
        if len(data) < 20:  # Much more aggressive - reduced from 50
            return None
        
        # Analyze Elliott Wave pattern
        pattern_data = await self.analyze_elliott_wave_pattern(symbol)
        
        # If service fails, use fallback mean reversion logic
        if not pattern_data:
            logger.warning(f"Elliott Wave service unavailable for {symbol}, using fallback logic")
            return generate_fallback_signal(symbol, data, self.name)
