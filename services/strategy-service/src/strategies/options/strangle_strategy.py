"""
Strangle Options Strategy
=========================
A strategy that buys or sells out-of-the-money call and put options.
Ideal for volatility trading with lower cost than straddles.
"""

import pandas as pd
import numpy as np
from typing import Dict, Any, Optional, List, Tuple
from datetime import datetime, timedelta
import logging
import random

from src.strategies.base import BaseStrategy
from src.core.types import TradeSignal
from src.utils.enhanced_logging import get_trading_logger
from src.services.market_data.options_data_service import OptionsDataService, OptionContract

logger = get_trading_logger()

class StrangleStrategy(BaseStrategy):
    """
    Strangle Options Strategy
    
    Features:
    - Long/short strangle for volatility trading
    - Lower cost than straddles
    - OTM strike selection
    - Earnings event trading
    - Defined risk and reward
    - Automated strike selection
    """
    
    def __init__(self, 
                 name: str = "Strangle",
                 days_to_expiration: int = 30,
                 profit_target_pct: float = 0.7,  # 70% of max profit
                 stop_loss_pct: float = 1.5,  # 1.5x max loss
                 max_risk_per_trade: float = 0.02,  # 2% of portfolio
                 min_delta: float = 0.2,  # Minimum delta for OTM selection
                 max_delta: float = 0.4,  # Maximum delta for OTM selection
                 min_strike_distance: float = 0.02,  # Minimum distance from current price
                 max_strike_distance: float = 0.08,  # Maximum distance from current price
                 strategy_type: str = "long",  # "long" or "short" strangle
                 earnings_days_before: int = 5,  # Days before earnings
                 earnings_days_after: int = 2):  # Days after earnings
        super().__init__(name)
        self.days_to_expiration = days_to_expiration
        self.profit_target_pct = profit_target_pct
        self.stop_loss_pct = stop_loss_pct
        self.max_risk_per_trade = max_risk_per_trade
        self.min_delta = min_delta
        self.max_delta = max_delta
        self.min_strike_distance = min_strike_distance
        self.max_strike_distance = max_strike_distance
        self.strategy_type = strategy_type
        self.earnings_days_before = earnings_days_before
        self.earnings_days_after = earnings_days_after
        
        # Initialize options service
        self.options_service = OptionsDataService()
        
        # Strategy state
        self.active_positions = {}
        self.position_history = []
    
    def check_market_conditions(self, data: pd.DataFrame, options_data: Optional[Dict] = None) -> bool:
        """Check if market conditions are suitable for strangle strategy"""
        
        if len(data) < 20:
            return False
        
        current_price = data['Close'].iloc[-1]
        
        # Check volatility conditions
        if 'ATR' in data.columns:
            atr = data['ATR'].iloc[-1]
            volatility = atr / current_price
            if volatility < 0.015:  # Less than 1.5% daily volatility
                return False
        
        # Check options data availability
        if not options_data:
            return False
        
        return True
    
    def select_otm_strikes(self, current_price: float, options_chain: List[OptionContract]) -> Optional[Tuple[OptionContract, OptionContract]]:
        """Select out-of-the-money call and put options"""
        
        # Find OTM options within distance range
        otm_calls = [opt for opt in options_chain 
                    if opt.option_type == 'call' and 
                    opt.strike > current_price * (1 + self.min_strike_distance) and
                    opt.strike < current_price * (1 + self.max_strike_distance)]
        otm_puts = [opt for opt in options_chain 
                   if opt.option_type == 'put' and 
                   opt.strike < current_price * (1 - self.min_strike_distance) and
                   opt.strike > current_price * (1 - self.max_strike_distance)]
        
        if not otm_calls or not otm_puts:
            return None
        
        # Select strikes based on delta requirements
        suitable_calls = [c for c in otm_calls if self.min_delta <= abs(c.delta) <= self.max_delta]
        suitable_puts = [p for p in otm_puts if self.min_delta <= abs(p.delta) <= self.max_delta]
        
        if not suitable_calls or not suitable_puts:
            return None
        
        # Select closest to current price within range
        call = min(suitable_calls, key=lambda x: x.strike)
        put = max(suitable_puts, key=lambda x: x.strike)
        
        return call, put
    
    def calculate_position_metrics(self, call: OptionContract, put: OptionContract, 
                                 current_price: float) -> Dict[str, Any]:
        """Calculate strangle position metrics"""
        
        if self.strategy_type == "long":
            total_cost = call.price + put.price
            breakeven_up = call.strike + total_cost
            breakeven_down = put.strike - total_cost
            max_loss = total_cost
            max_profit = float('inf')
        else:  # short strangle
            total_credit = call.price + put.price
            breakeven_up = call.strike + total_credit
            breakeven_down = put.strike - total_credit
            max_loss = float('inf')
            max_profit = total_credit
        
        # Calculate Greeks
        total_delta = call.delta + put.delta
        total_gamma = call.gamma + put.gamma
        total_theta = call.theta + put.theta
        total_vega = call.vega + put.vega
        
        return {
            'total_cost': call.price + put.price,
            'breakeven_up': breakeven_up,
            'breakeven_down': breakeven_down,
            'max_loss': max_loss,
            'max_profit': max_profit,
            'total_delta': total_delta,
            'total_gamma': total_gamma,
            'total_theta': total_theta,
            'total_vega': total_vega,
            'call_strike': call.strike,
            'put_strike': put.strike,
            'call_price': call.price,
            'put_price': put.price,
            'strategy_type': self.strategy_type
        }
    
    def check_earnings_timing(self, symbol: str) -> bool:
        """Check if we're in the right timing window for earnings trade"""
        # This would integrate with earnings calendar
        # For now, simulate earnings timing
        return random.random() < 0.15  # 15% chance of earnings timing
    
    def calculate_iv_percentile(self, symbol: str, options_chain: List[OptionContract]) -> float:
        """Calculate implied volatility percentile"""
        if not options_chain:
            return 0.5
        
        # Calculate average IV
        avg_iv = np.mean([opt.implied_volatility for opt in options_chain if opt.implied_volatility > 0])
        
        # Simplified IV percentile calculation
        # In practice, this would compare to historical IV distribution
        if avg_iv > 0.4:
            return 0.8
        elif avg_iv > 0.3:
            return 0.6
        elif avg_iv > 0.2:
            return 0.4
        else:
            return 0.2
    
    def _calculate_confidence(self, data: pd.DataFrame, position: Dict, 
                            iv_percentile: float) -> float:
        """Calculate signal confidence"""
        
        confidence = 0.5  # Base confidence
        
        # IV percentile factor
        if self.strategy_type == "short":
            # Short strangle prefers high IV
            if iv_percentile > 0.7:
                confidence += 0.2
            elif iv_percentile > 0.5:
                confidence += 0.1
        else:
            # Long strangle prefers low IV
            if iv_percentile < 0.3:
                confidence += 0.2
            elif iv_percentile < 0.5:
                confidence += 0.1
        
        # Volatility factor
        if 'ATR' in data.columns:
            atr = data['ATR'].iloc[-1]
            current_price = data['Close'].iloc[-1]
            volatility = atr / current_price
            if volatility > 0.025:  # High volatility
                confidence += 0.15
        
        # Technical indicators
        if 'RSI' in data.columns:
            rsi = data['RSI'].iloc[-1]
            if 30 <= rsi <= 70:  # Neutral RSI
                confidence += 0.1
        
        # Volume factor
        if 'Volume' in data.columns:
            avg_volume = data['Volume'].rolling(20).mean().iloc[-1]
            current_volume = data['Volume'].iloc[-1]
            if current_volume > avg_volume * 1.3:  # High volume
                confidence += 0.05
        
        return min(0.95, confidence)
    
    async def generate_signal(self, symbol: str, data: pd.DataFrame, 
                            options_data: Optional[Dict[str, Any]] = None) -> Optional[TradeSignal]:
        """Generate Strangle Strategy signal"""
        
        if len(data) < 20:
            return None
        
        current_price = data['Close'].iloc[-1]
        
        # Check market conditions
        if not self.check_market_conditions(data, options_data):
            return None
        
        # Check earnings timing
        if not self.check_earnings_timing(symbol):
            return None
        
        # Get options chain
        options_chain = self.options_service.get_liquid_options(symbol, min_volume=10)
        if not options_chain:
            return None
        
        # Select OTM strikes
        otm_options = self.select_otm_strikes(current_price, options_chain)
        if not otm_options:
            return None
        
        call, put = otm_options
        
        # Calculate position metrics
        position = self.calculate_position_metrics(call, put, current_price)
        
        # Calculate IV percentile
        iv_percentile = self.calculate_iv_percentile(symbol, options_chain)
        
        # Calculate confidence
        confidence = self._calculate_confidence(data, position, iv_percentile)
        
        # Only trade if confidence is sufficient
        if confidence < 0.6:
            return None
        
        # Generate signal
        action = "SHORT_STRANGLE" if self.strategy_type == "short" else "LONG_STRANGLE"
        
        signal = TradeSignal(
            symbol=symbol,
            action=action,
            quantity=1,  # One strangle position
            price=current_price,
            timestamp=datetime.now(),
            strategy=self.name,
            confidence=confidence,
            metadata={
                'call_strike': call.strike,
                'put_strike': put.strike,
                'total_cost': position['total_cost'],
                'breakeven_up': position['breakeven_up'],
                'breakeven_down': position['breakeven_down'],
                'max_loss': position['max_loss'],
                'max_profit': position['max_profit'],
                'iv_percentile': iv_percentile,
                'total_delta': position['total_delta'],
                'total_gamma': position['total_gamma'],
                'total_theta': position['total_theta'],
                'total_vega': position['total_vega'],
                'signal_type': 'strangle',
                'strategy_type': self.strategy_type,
                'position_size': self.max_risk_per_trade / position['max_loss'] if position['max_loss'] != float('inf') else 0,
                'profit_target': position['max_profit'] * self.profit_target_pct if position['max_profit'] != float('inf') else position['total_cost'] * self.profit_target_pct,
                'stop_loss': position['max_loss'] * self.stop_loss_pct if position['max_loss'] != float('inf') else position['total_cost'] * self.stop_loss_pct
            }
        )
        
        logger.info(f"Strangle signal: {symbol} {self.strategy_type} (confidence: {confidence:.3f}, "
                   f"cost: ${position['total_cost']:.2f}, IV percentile: {iv_percentile:.1%})")
        
        return signal 