#!/usr/bin/env python3
"""
Iron Condor Simulation: INTC, AMD, PYPL
Test Iron Condor performance on specific tech stocks
"""
...
class IronCondorStockSimulator:
    """Simulate Iron Condor strategies on specific stocks"""
    ...
    def __init__(self):
        ...
        # Stock-specific Iron Condor configurations
        self.stocks = {
            "INTC": {
                ...
                "iron_condor_config": {
                    ...
                    "max_daily_trades": 1,
                    "description": "Intel Iron Condor - moderate volatility"
                }
            },
            "AMD": {
                "current_price": 120.0,     # AMD around $120
                "volatility": 0.45,         # 45% annual volatility
                "daily_volatility": 0.028,  # 2.8% daily volatility
                "iron_condor_config": {
                    ...
                    "max_position_size": 0.10,
                    "max_daily_trades": 1,
                    "description": "AMD Iron Condor - high volatility"
                }
            },
            "PYPL": {
                "current_price": 65.0,       # PayPal around $65
                ...
                "iron_condor_config": {
                    ...
                    "avg_loss": -0.032,        # 3.2% average loss
                    "max_position_size": 0.10,
                    "max_daily_trades": 1,
                    "description": "PayPal Iron Condor - moderate-high volatility"
                }
            }
        }
    ...
    def get_regime_multiplier(self, regime: str, stock: str) -> float:
        """Get performance multiplier based on market regime and stock volatility"""
        ...
        base_multipliers = {
            ...
            "volatile": 0.80,    # 20% reduction in volatile markets
            "range_bound": 1.20   # 20% boost in range-bound markets
        }
        
        base_multiplier = base_multipliers.get(regime, 1.0)
        
        # Adjust for stock volatility
        if stock_vol > 0.40:  # High volatility stocks (AMD)
            volatility_adjustment = 0.95  # 5% reduction for high vol
        elif stock_vol > 0.35:  # Medium-high volatility (PYPL)
            volatility_adjustment = 1.0   # No adjustment
        else:  # Medium volatility (INTC)
            volatility_adjustment = 1.05  # 5% boost for medium vol
    ...
    def simulate_iron_condor_trade(self, stock: str, day: int) -> Dict[str, Any]:
        ...
        if is_winner:
            ...
            # Cap maximum win at 4% (Iron Condor has limited upside)
            final_return = min(final_return, 0.04)
            ...
            # Cap maximum loss at 5% (Iron Condor has limited downside)
            final_return = max(final_return, -0.05)
    ...
    def run_simulation(self, days: int = 730) -> Dict[str, Any]:
        """Run Iron Condor simulation on INTC, AMD, PYPL"""
        logger.info(f"🚀 Starting Iron Condor simulation on INTC, AMD, PYPL")
...
async def main():
    """Run the Iron Condor stock simulation"""
    simulator = IronCondorStockSimulator()
    
    print("=" * 80)
    print("🚀 IRON CONDOR SIMULATION: INTC, AMD, PYPL")
    print("=" * 80)
    print()
    ...
    print()
    print("=" * 80)
    print("🎯 CONCLUSION: Iron Condor performance on specific tech stocks")
    print("=" * 80)
